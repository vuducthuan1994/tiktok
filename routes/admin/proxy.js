...
module.exports = function() {
    ...
    router.get('/', function(req, res) {
        Proxys.find({}, function(err, proxys) {
            res.render('admin/pages/proxy/index', { messages: req.flash('message'), title: "Proxy", proxys: proxys.map(proxy => proxy.toJSON()), layout: 'admin.hbs' });
        }).sort({ updated_date: -1 });
    });
    router.get('/add-proxy', isAuthenticated, function(req, res) {
        // res.locals.originalUrl = req.originalUrl;
        res.render('admin/pages/proxy/add-proxy', { title: "Add Proxy", layout: 'admin.hbs' });
    });
    ...
    router.post('/add-proxy', isAuthenticated, function(req, res) {
        ...
        if (proxy.length < 5) {
            ...
            res.render('admin/pages/proxy/add-proxy', { title: "Proxy", layout: 'admin.hbs' });
        } else {
            Proxys.create({
